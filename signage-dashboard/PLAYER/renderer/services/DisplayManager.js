...
export class DisplayManager {
   ...
   *   - onPlaylistChanged: Called when playlist_id changes
  ...
  async setupRealtimeEvents(callbacks = {}) {
    try {
      ...
      // 2. Listen to playlist items changes (only if we have a playlist)
      // This will be set up dynamically when playlist is loaded
      this.setupPlaylistItemsListener = (playlistId) => {
        if (!playlistId) return;

        logInfo("Setting up playlist items listener for:", playlistId);

        // Listen to changes in this specific playlist
        const playlistChannel = window.SupaRT.onPlayListRowChange(playlistId, (payload) => {
          ...
          if (callbacks.onPlaylistItemsChanged) {
            ...
            // Default behavior: reload playlist
            logInfo("Playlist items changed, reloading content...");
            setTimeout(() => {
              if (callbacks.reloadPlaylist) {
                callbacks.reloadPlaylist();
              } else {
                location.reload();
              }
            }, 1000);
        ...
        // Listen to playlist item deletions
        const deleteChannel = window.SupaRT.onPlayListDelete((payload) => {
          logInfo("Playlist item deleted:", payload);
          
          // Check if deleted item belongs to our playlist
          if (payload.old?.playlist_id === playlistId) {
            logInfo("Item deleted from our playlist, reloading...");
            if (callbacks.onPlaylistItemsChanged) {
              callbacks.onPlaylistItemsChanged(playlistId, payload);
            } else {
              setTimeout(() => {
        ...
        this.realtimeChannels.push(deleteChannel);
      };

      // 4. Setup screen events (physical monitor changes)
