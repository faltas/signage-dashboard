...
export const PlayerState = {
  ...
  setScreens(screens) {
    ...
    // Initialize empty playlists for new screens if they don't exist
    this.screens.forEach(screen => {
      if (!this.screenPlaylists[screen.id]) {
        this.screenPlaylists[screen.id] = { items: [], playlistId: null };
      }
    });
    logInfo(`[STATE] Screens updated: ${this.screens.length}`);
  },

  updateScreenPlaylist(screenId, playlistId, items) {
    if (!this.screenPlaylists[screenId]) {
      this.screenPlaylists[screenId] = {};
    }
    this.screenPlaylists[screenId].playlistId = playlistId;
    this.screenPlaylists[screenId].items = items || [];
    logInfo(`[STATE] Updated playlist for screen ${screenId}`);
  },
